...
#define N_RAIN_DRIPLET 18
#define TRAIL_LEN 5

struct rain_driplet rain_driplets[N_RAIN_DRIPLET] = {[0 ... N_RAIN_DRIPLET-1] = {0,TRAIL_LEN + N_ROWS + 1}  };
    ...
    *buffer = 0;
    for (uint8_t i = 0; i < N_RAIN_DRIPLET ; i++) {
        if (led_pos_eye(buffer_index)) {
            if ((16 - led_pos_cart_x[buffer_index]) == rain_driplets[i].x) {
                if((TRAIL_LEN + N_ROWS) < rain_driplets[16 - led_pos_cart_x[buffer_index]].y) {
                    ...
                    if (0 <= rain_y_diff) {
                        if (TRAIL_LEN > rain_y_diff) {
                            *buffer = 0x1F >> rain_y_diff;
                        }
                    }
                }
            }
        } else {
            if (led_pos_cart_x[buffer_index] == rain_driplets[i].x) {
...
void rain_beat_start(void) {
    for(uint8_t i = 0; i < 2 ; i++) {
        uint8_t next_drip = random_rain % 18;
        random_rain = (random_rain * 196314165) + 907633515;
        while (0 != rain_driplets[next_drip].y) {
            if ((TRAIL_LEN + N_ROWS + 1) == rain_driplets[next_drip].y) {
                rain_driplets[next_drip].y = 0;
                rain_driplets[next_drip].x = next_drip;
                break;
            }
            random_rain = (random_rain * 196314165) + 907633515;
            next_drip = random_rain % 18;
        }
    }
}
